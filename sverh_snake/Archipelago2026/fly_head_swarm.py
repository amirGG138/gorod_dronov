#!/usr/bin/env python3
"""Головной дрон (лидер) на зачётной попытке «Змейки» — версия ДЛЯ РОЯ.

═══════════════════════════════════════════════════════════════════════════════
ЭТО КОПИЯ `fly_head.py`. Отличий от неё ровно пять, все — маячок цепочки:

  1. `import socket`, Nodename и настройки TEAM / BEACON_* / JOIN_WAIT_S / TAILS /
     LAND_CALL_*;
  2. секция МАЯЧОК ЦЕПОЧКИ ниже: beacon(), land_all(), joined(), wait_join();
  3. в `watch()` вместо голого `sleep(HOVER_S)` — маячок и ожидание хвоста;
  4. вызовы `beacon()` в `main()`: после скана и в трёх циклах перелётов;
  5. `land_all()` в `finally` блока `main()` — команда рою садиться (п. 2.1.5).

Полёт самого лидера не меняется НИЧЕМ. `fly_head.py` оставлен нетронутым, чтобы на
площадке можно было поднять одиночного лидера заведомо проверенной программой.

ПРАВКУ, СДЕЛАННУЮ В ОДНОМ ФАЙЛЕ, НАДО РУКАМИ ПЕРЕНЕСТИ ВО ВТОРОЙ. Проверка:
`diff fly_head.py fly_head_swarm.py` — расхождений должно быть ровно пять.
═══════════════════════════════════════════════════════════════════════════════
...
Хвостовые дроны (`fly_tail1.py`…`fly_tail3.py`) летят отдельными программами и
слушают маячок этой: распознав яблоко N, лидер сообщает об этом в эфир, дрон N
взлетает с земли, а лидер висит на месте, пока тот не доложит, что встал в хвост
(п. 2.1.2).
...
# Маячок цепочки. UWB на этих дронах нет, а камера смотрит вниз — хвост не видит ни
# лидера, ни соседа. Поэтому место в строю передаётся по Wi-Fi: п. 2.6.7 разрешает
# внутри команды любые беспроводные каналы. Обмен предельно простой, три строки —
# ключ команды, буква, полезная часть и общий хвост «<запуск> <счётчик>»:
...
# Ни подтверждений, ни повторов, ни таймаутов: потерянная датаграмма заменяется
# следующей. Ключ команды отсекает чужие программы на том же порту. Номер запуска
# отсекает зависший экземпляр НАШЕЙ программы: хвост держится за наибольший
# услышанный номер, так свежий пуск всегда побеждает старый — чего SO_REUSEADDR не
# даёт. Счётчик — сквозной номер датаграммы: по нему отбрасываются переставленные и
# повторные пакеты, иначе устаревший «H» мог бы ложно отмотать след назад.
TEAM = "SVRH26"         # ключ команды; тот же в fly_tail*.py
BEACON_PORT = 55055     # порт маячка; тот же в fly_tail*.py
...
# вписываются адреса хвостов — та же датаграмма уйдёт каждому лично, тем же сокетом.
# Проверка на площадке: пинг между бортами идёт, а «лидер: маячка не слышно» —
# значит нужен этот список. Пустой — только широковещание, как раньше.
# Лишним он не будет: дубль по номеру датаграммы хвост отбрасывает сам.
PEERS = []              # напр. ["192.168.1.127", "192.168.1.128", "192.168.1.129"]
# П. 2.1.2: цепочка висит на яблоке, пока новый дрон не встанет в хвост. Ждать без
# предела нельзя — не взлетевший дрон повесил бы всю попытку. И это же время идёт из
# трёх минут на сборку (SEARCH_LIMIT_S): три ожидания по 45 с — это 135 с из 180, и
# облетать поле в поисках оставшихся яблок будет уже некогда. Подбирать на площадке.
JOIN_WAIT_S = 45.0      # ждать доклада хвоста не дольше этого, с
# Ждать имеет смысл только тех, кто вообще есть: распознанных яблок может оказаться и
# больше трёх (ложное пятно на полу), а четвёртого дрона в команде нет — и лидер
# простоял бы на нём весь JOIN_WAIT_S, вычтя эти секунды из трёх минут на сборку.
TAILS = 3               # сколько хвостовых дронов в цепочке: fly_tail1…fly_tail3
# П. 2.1.5 требует СИНХРОННОЙ посадки роя. Общий таймер её не даёт: программы
# запускаются не одномоментно, и часы у дронов разъезжаются. Поэтому посадку
# командует лидер, а команда повторяется — одна потерянная датаграмма не должна
# оставить дрон в воздухе.
LAND_CALL_N = 6         # столько раз повторить команду посадки в эфир
LAND_CALL_S = 0.3       # пауза между повторами, с
...
# ═══════════════════════════════════════════════════════════════════════
#
# Один сокет и на передачу, и на приём: широковещание возвращается и отправителю,
# поэтому лидер слышит собственные «H» и «L». Разбираются только «T» — доклады хвостов.

JOINED = set()          # номера хвостов, доложивших «встал в хвост»
# Номер запуска: по нему хвосты отличают этот экземпляр программы от зависшего с
# прошлого раза. Секунды текущих суток — числу расти между запусками достаточно, а
# в пять знаков оно всегда влезает.
RUN = int(time.time()) % 86400
SEQ = 0                 # сквозной счётчик датаграмм этого запуска
...
try:
    ...
    # Порт занят — докладов хвостов лидер не услышит и будет висеть на каждом яблоке
    # весь JOIN_WAIT_S. Полёт от этого не срывается, но знать надо до вылета.
    print(f"!!! маячок: порт {BEACON_PORT} не открылся ({exc}) — "
          f"докладов хвостов не будет", flush=True)


def say(line):
    """Отправить строку маячка, приписав ключ команды, номер запуска и счётчик.

    Односторонне и без подтверждений: не дошло — хвост возьмёт следующую датаграмму.
    Отказ сокета печатается и игнорируется: полёт лидера от маячка не зависит.
    """
    global SEQ
    SEQ += 1
    msg = f"{TEAM} {line} {RUN} {SEQ}".encode()
    # Широковещательно всем и, если PEERS заполнен, ещё и каждому лично: при
    # разных подсетях или изоляции клиентов доходит только второе. Дубль хвост
    # отбрасывает по номеру датаграммы, так что слать обоими путями безопасно.
    for addr in [BEACON_ADDR] + PEERS:
        try:
            _BEACON.sendto(msg, (addr, BEACON_PORT))
        except OSError as exc:
            print(f"          маячок не ушёл на {addr}: {exc}", flush=True)


def beacon(reached, apples_seen):
    """Сказать хвостам, какой узел лидер ПРОШЁЛ и сколько яблок засчитано.

    Именно пройденный, а не тот, к которому летим: след — это узлы, где голова
    действительно была (п. 2.1.4, «след в след»). Скажи мы цель заранее — на
    пропущенном узле хвосты пошли бы туда, где лидера нет и не будет.
    """
    say(f"H {reached} {apples_seen}")


def land_all():
    """Скомандовать рою посадку (п. 2.1.5) и дать команде уйти в эфир.

    Общий таймер синхронной посадки не даёт: четыре программы запускаются не
    одномоментно, да и часы у дронов расходятся. Команда снимает и то, и другое —
    хвосты садятся по ней, а не по своим часам, и укладываются в секунду-две.

    Повторов LAND_CALL_N, потому что подтверждений в протоколе нет: одна потерянная
    датаграмма не должна оставить дрон висеть до его резервного таймера.
    """
    print(f"КОМАНДА РОЮ: ПОСАДКА (×{LAND_CALL_N})", flush=True)
    for _ in range(LAND_CALL_N):
        say("L")
        time.sleep(LAND_CALL_S)


def joined():
    """Забрать доклады хвостов «встал в хвост». Не блокирует.
    ...
    Чужое отсеивается двумя полями: ключ команды отбрасывает программы других команд
    на том же порту, номер запуска — доклады хвостов, оставшиеся от прошлого пуска.
    """
    while True:
        try:
            ...
            return
        parts = data.split()
        # "<ключ> T <номер хвоста> <запуск> <счётчик>"
        if len(parts) != 5 or parts[0] != TEAM.encode() or parts[1] != b"T":
            continue                    # свой же «H»/«L» или чужая датаграмма на порту
        try:
            number, run = int(parts[2]), int(parts[3])
        except ValueError:
            continue
        if run == RUN:
            JOINED.add(number)
...
def wait_join(spot, number, deadline=None):
    """Висеть на яблоке, пока хвост `number` не доложит, что встал (п. 2.1.2).

    Регламент требует остановки всей цепочки, пока новый дрон не займёт узел в
    хвосте: иначе выходит «погоня» за уходящим строем и точка входа теряет смысл.
    Ждём не дольше JOIN_WAIT_S — дрон мог не взлететь вовсе, и вешать на нём всю
    попытку нельзя.

    `deadline` тот же, что у поиска: три минуты на сборку (п. 2.1.3) — это ОБЩЕЕ
    время, и ожидание хвоста тратит именно его. Без этой границы три ожидания по
    45 с вынесли бы лидера за лимит на три четверти минуты.

    Маячок шлётся каждый виток ожидания: хвост, только что оторвавшийся от земли,
    должен сразу узнать, где стоит голова, а не ждать, пока цепочка тронется.
    """
    limit = time.time() + JOIN_WAIT_S
    if deadline is not None:
        limit = min(limit, deadline)
    while time.time() < limit:
        joined()
        if number in JOINED:
            print(f"          дрон {number} встал в хвост", flush=True)
            return True
        beacon(spot, number)
        time.sleep(0.5)
    print(f"          дрон {number} не доложил за {JOIN_WAIT_S:.0f} с — идём дальше",
          flush=True)
    return False
...
def watch(drone, base, fruit, found, deadline=None):
    ...
    for colour, x, y in fruit:
        ...
        if known is None:
            ...
            fresh = number <= TAILS      # четвёртому яблоку взлетать уже некому
            tail = f" — ждём дрон {number}" if fresh else " — хвостов больше нет"
        else:
            known[ATTEMPTS] += 1
            if not reached:
            ...
            tail = f" — зависаем {HOVER_S} с"
            fresh = False  # дрон под это яблоко взлетел ещё когда его распознали

        print(f">>> ЯБЛОКО {number}/{APPLES_TOTAL}: {colour} {at(where)}{note}{tail}",
              flush=True)
        # Узел зависания — метка под дроном СЕЙЧАС, а не та, от которой яблоко было
        # замечено: подлёт к яблоку уводит голову с неё почти на целый узел, и хвост,
        # отсчитавший своё место от старой, встал бы не в полуметре позади, а где
        # придётся — ровно в тот момент, когда цепочка стоит и дистанцию удобнее всего
        # мерить (п. 2.1.4). Метки под дроном не видно — остаётся прежняя, других
        # данных всё равно нет; 0 — законный ID, поэтому проверка на None, а не «or».
        stand = underneath(drone)
        if stand is None:
            stand = base[0]
        # Маячок: «яблоко N распознано» — это и есть команда на взлёт дрону N (п. 2.1.2).
        beacon(stand, number)
        time.sleep(HOVER_S)                     # даём хвосту услышать и оторваться
        if fresh:
            # П. 2.1.2: цепочка не двигается, пока новый дрон не встанет в хвост. Это
            # ещё и единственное, что даёт хвосту время догнать строй: с прежними
            # двумя секундами он не успевал даже набрать высоту. Ограничено теми же
            # тремя минутами, что и весь поиск, — ожидание идёт из них же.
            wait_join(stand, number, deadline)
        return True
    return False
...
def main():
    drone = sverk_interfaces.init(Nodename="fly_head_swarm")
    patch_yuv(drone)
    try:
        ...
        if start is None:
            ...
            # Начало следа: хвосты должны знать, откуда пошла голова, ещё до первого
            # её перелёта — иначе первый услышанный ими узел окажется вторым.
            beacon(start, 0)
            ...
            for target in route:
                ...
                try:
                    # Маячок — ПОСЛЕ прилёта и только при подтверждённом: след должен
                    # состоять из узлов, где голова была, а не куда собиралась. На
                    # пропущенном узле след просто не растёт, и хвосты стоят на месте,
                    # вместо того чтобы идти туда, где лидера нет.
                    if goto(drone, target, found, deadline,
                            tol=SEARCH_TOL, tries=SEARCH_TRIES):
                        beacon(target, len(found))
            ...
            for _ in range(GRID * 2):
                ...
                try:
                    ...
                    if goto(drone, path[0], found, back_by, hunt=False):
                        here, stuck = path[0], 0
                        # След продолжается и на заходе: хвосты идут за головой тем же
                        # порядком, а не висят там, где кончился поиск.
                        beacon(here, len(found))
                        continue
                except FlightAborted:
                    raise
                except Exception as exc:
        ...
        try:
            # Сначала команда рою, потом своя посадка: хвосты должны пойти вниз
            # вместе с лидером (п. 2.1.5), а не каждый по своему резервному таймеру.
            # Стоит здесь, а не после фигуры, чтобы сработать на ЛЮБОМ выходе —
            # включая Ctrl+C, KILL SWITCH и FlightAborted.
            land_all()
