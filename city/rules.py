...
def plan_reasons(
    ...
    `spot` — клетка подъезда, если её предложила модель и пропустили правила.
    Передавать её сюда обязательно: иначе лог назовёт судье одну клетку, а ровер
    поедет на другую.
    """
    default = field.approach(sc.fire_cell, sc.tower)
    if default is None:
        raise RouteBlocked(f"к пожару {list(sc.fire_cell)} не подъехать: соседние клетки заняты")
    chosen = as_cell(spot) if spot is not None else default
    why = (
        "из соседних выбрана ближайшая к башне"
        if chosen == default
        else "клетку предложила модель, правила проверили её допустимость"
    )
    return [
        ...
        f"тушим с клетки {list(chosen)}: в клетку пожара не въезжаем, {why}",
    ]
