...
FIRE_BGR = (40, 30, 190)
FIRE_M = 0.045  # сторона жетона на поле, м — по фотографиям
...
class SceneSpec:
    ...
    def render(self, xy: Sequence[float], alt: float) -> bytes | None:
        ...
        if self.fire_cell is not None:
            _draw_fire(frame, anchor, self.field.cell_to_m(self.fire_cell))
...
def _draw_fire(frame, anchor: Anchor, xy: tuple[float, float]) -> None:
    radius = max(2, int(round(FIRE_M / 2 / anchor.mpp)))
    u, v = anchor.to_pixel(*xy)
    if not _visible(frame, u, v, radius):
        return
    cv2.circle(frame, (int(round(u)), int(round(v))), radius, FIRE_BGR, -1)
