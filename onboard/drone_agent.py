...
* Дальномер меряет расстояние до поверхности ПРЯМО ПОД ДРОНОМ (фрейм terrain), а поле
  разновысотное: дома и крыши-площадки подняты на 0,5–0,8 м. Крыша — одна клетка
  80×80 см, то есть от метки до края 40 см, а точки обзора облёта отстоят на 50 см:
  дрон гарантированно вылетает за край. Замер скачком растёт на высоту дома, автопилот
  принимает это за собственный подъём и проседает; на возврате — подскакивает.
  Лечение: скачок замера переносится в ЦЕЛЬ, а не в движение — новая цель во фрейме
  terrain больше ровно на величину скачка, и рассогласования у автопилота не остаётся
  (класс Terrain, alt_keeper). Делается немедленно и во время перелёта тоже.
  Выключается --no-alt-hold.
...
# Высота площадки над полом поля, м, по клетке (docs/field-map/README.md: маркеры
# 50/60 на 0,515 м, 62/7 на 0,825 м — все четыре площадки на крышах домиков).
# На железе карта не проверялась: в первый день мерить рулеткой и задавать --pad-z.
PAD_Z = {(1, 4): 0.515, (4, 4): 0.515, (1, 1): 0.825, (4, 1): 0.825}
...
def parse_levels(text: str) -> list[float]:
    """«0.515,0.825» -> [0.515, 0.825]. Пустая строка и мусор дают пустой список.

    Мусор глотаем молча не из лени: ключ этот задаётся руками на площадке, и падать
    из-за лишней запятой в момент, когда дрон уже на крыше, — худший из исходов.
    """
    out: list[float] = []
    for chunk in (text or "").replace(";", ",").split(","):
        ...
        try:
            ...
            say(f"уровень поверхности «{chunk}» не число — пропускаю")
    return out


# ═══════════════════════════════════════════════════════════════════════════
#  РЕЛЬЕФ: здание под дроном против дрейфа самого дрона
# ═══════════════════════════════════════════════════════════════════════════
#
# Дальномер дрона меряет расстояние до ПОВЕРХНОСТИ ПРЯМО ПОД НИМ (в терминах
# документации это фрейм terrain), а не высоту над полом. Поле у нас разновысотное:
# дома и крыши-площадки подняты на 0,5–0,8 м (docs/field-map/). Стоит дрону выйти за
# край крыши, как замер меняется на всю высоту дома — и автопилот, приняв это за
# падение, добирает ту же высоту моторами. Дрон уходит вверх, ничего не потеряв.
#
# ПРАВИЛО ОДНО: резкий скачок замера — это смена поверхности, и переносить его надо
# в ЦЕЛЬ, а не в движение дрона. Замер вырос на полметра — значит и просить у
# автопилота надо на полметра больше: рассогласование у него остаётся нулевым, и он
# никуда не идёт. Отличить скачок от собственного ухода дрона можно по скорости, и
# только по ней: сам дрон ограничен скоростью набора (--climb-speed) и за такт не
# проходит и десяти сантиметров, а край дома меняет замер мгновенно.
#
# Реакция обязана быть немедленной: автопилот отрабатывает скачок за доли секунды.
# Поэтому подтверждения вторым замером здесь НЕТ — оно стоило бы целого такта, а
# защиту от вранья дальномера даёт список известных уровней (см. ниже).
#
# Карта поля (docs/field-map/) говорит, какие уровни поверхности вообще бывают: пол
# и своя крыша, разнесённые на полметра. Список нужен не для распознавания скачка, а
# для двух других вещей: подтвердить, что накопленный уровень правдоподобен, и НЕ
# править дрейф, пока дрон в переходе между уровнями — на кромке дома дальномер
# успевает выдать промежуточные значения, и раньше они выглядели плавным уходом,
# заставляя корректор вредить.


class Terrain:
    """Где пол, если под дроном то крыша, то земля.

    Держит `ground` — уровень поверхности под дроном относительно той площадки, с
    которой взлетали (крыша дома = +0,5 м, пол поля = −0,5 м, если стартовали с
    крыши). Высота над этим уровнем — то, что показывает дальномер, а высота над
    точкой взлёта = `ground + замер`, и именно её надо держать постоянной.

    Класс намеренно ничего не знает ни про дрон, ни про ROS: только числа и время.
    Так его можно проверить тестами до полёта — на живом борте эта логика проверяется
    один раз и дорого.
    """

    def __init__(self, step: float, settle: float, gap: float, climb_speed: float,
                 ground_max: float, levels=()) -> None:
        self.step = step                # скачок больше этого = смена поверхности, м
        self.settle = settle            # допуск «уровень совпал с известным», м
        self.gap = gap                  # перерыв в замерах, после которого не судим, с
        self.climb_speed = climb_speed  # быстрее этого дрон высоту сам не меняет, м/с
        self.ground_max = ground_max    # предел правдоподобия для уровня поверхности, м
        # Какие уровни поверхности бывают под этим дроном, в метрах от своей площадки:
        # 0 — она сама, −0,5 — пол поля под ней. Пустой список означает «сверять не с
        # чем», и тогда класс работает на одних скачках, как раньше.
        self.levels = tuple(sorted({round(float(v), 3) for v in levels}))
        self.reset()

    def reset(self) -> None:
        """Забыть всё: новый полёт считает уровень своей площадки нулём."""
        self.ground = 0.0
        self.height: float | None = None   # последний принятый замер до поверхности, м
        self.at = 0.0                      # время этого замера
        self.steps = 0                     # сколько ступенек приняли за полёт
        self.clipped = False               # уровень упирался в предел правдоподобия
        self.known = True                  # уровень совпал с одним из известных

    @property
    def agl(self) -> float | None:
        """Высота над уровнем взлётной площадки, м. None — замеров ещё не было."""
        return None if self.height is None else self.ground + self.height

    def target(self, alt: float, min_clear: float, max_alt: float) -> float:
        """Что просить у дальномерного фрейма, чтобы висеть на alt над площадкой.

        Над крышей дома это меньше alt ровно на высоту дома — в этом весь смысл:
        просить «полтора метра над поверхностью», стоя над домом, значит подняться.

        Зазор снизу не формальность: уровень поверхности мы оцениваем сами и могли
        ошибиться, а команда «висеть в 10 см над крышей» ошибку уже не переживёт.
        """
        return max(min_clear, min(max_alt, alt - self.ground))

    def update(self, height: float, now: float, alt: float) -> str:
        """Принять замер до поверхности. Что это было — в возвращённом слове.

        `alt` — высота, на которой дрон ДОЛЖЕН висеть над своей площадкой; нужна
        только после перерыва в замерах, чтобы выбрать уровень, а не гадать.

        first — первый замер, сравнивать не с чем;
        ok — обычный замер, высоту можно править;
        step — под дроном сменилась поверхность, уровень пересчитан НЕМЕДЛЕННО;
        resync — после перерыва уровень опознан заново;
        gap — перерыв был долгим, а уровень опознать не удалось;
        bad — замер негодный (не число, отрицательный).
        """
        if height is None or height != height or height in (float("inf"), float("-inf")):
            return "bad"
        if height < 0.0:
            return "bad"

        if self.height is None:
            self.height, self.at = height, now
            self._snap()
            return "first"

        dt = now - self.at
        if dt > self.gap:
            kind = self._resync(height, alt)
            self.height, self.at = height, now
            return kind

        # Сколько дрон физически мог пройти сам за это время, плюс порог на шум.
        own = self.step + self.climb_speed * max(dt, 0.0)
        delta = height - self.height
        self.height, self.at = height, now
        if abs(delta) > own:
            # Скачок. Поверхность поднялась ровно настолько, насколько уменьшилось
            # расстояние до неё, а высота дрона над полем не изменилась вовсе.
            # Принимаем сразу, без подтверждения: подтверждение стоит такта, а
            # автопилот к тому времени уже отработает скачок моторами.
            self.ground -= delta
            self._clip()
            self.steps += 1
            self._snap()
    ...
    def _snap(self) -> None:
        """Притянуть уровень к ближайшему известному — или честно признать, что не узнали.

        Промах меньше допуска — это накопившийся шум дальномера, его убираем. Промах
        больше — либо дрон на кромке дома и переход ещё не закончился, либо карта
        поля врёт. В обоих случаях `known` снимается, и дрейф до выяснения не правится:
        неправильная поправка хуже отсутствующей.
        """
        if not self.levels:
            self.known = True            # сверять не с чем: работаем на одних скачках
            return
        best = min(self.levels, key=lambda lv: abs(lv - self.ground))
        if abs(best - self.ground) <= self.settle:
            self.ground = best
            self.known = True
        else:
            self.known = False

    def _resync(self, height: float, alt: float) -> str:
        """После перерыва в замерах: не гадать, а выбрать уровень, объясняющий замер.

        За перерыв дрон мог и пролететь край дома, и просесть — по двум замерам этого
        не различить, и прежняя догадка «раз молчали, значит высоту держал» списывала
        на рельеф весь реальный уход. Зато известно, на какой высоте дрон ДОЛЖЕН
        висеть, и известен список уровней: они разнесены на полметра, а промахивается
        дрон на сантиметры — выбор получается однозначным.
        """
        if not self.levels:
            return "gap"                 # сверять не с чем — уровень оставляем как был
        best = min(self.levels, key=lambda lv: abs(lv + height - alt))
        if abs(best + height - alt) <= self.step:
            self.ground = best
            self.known = True
            return "resync"
        self.known = False
    ...
    def _clip(self) -> None:
        """Уровень поверхности за пределами правдоподобия — это накопленная ошибка.

        Дома на поле ниже метра, и уползший за ground_max уровень означает, что
        ступеньки считались неверно. Зажимаем и помечаем: врать про высоту опаснее,
        чем признаться, что оценка испортилась.
        """
        if abs(self.ground) > self.ground_max:
            self.ground = math.copysign(self.ground_max, self.ground)
            self.clipped = True
...
class Agent:
    ...
    def __init__(self, args) -> None:
        ...
        # Высота: уровень поверхности под дроном (крыша это или пол) и счётчик отказов
        # дальномера. Отказы копятся подряд: одиночный ничего не значит, а пять
        # означают, что замера нет вовсе и держать высоту не по чему.
        #
        # Уровни считаются ОТ СВОЕЙ ПЛОЩАДКИ, потому что от неё же отсчитывается
        # рабочая высота: сама она 0, пол поля под ней — минус её высота над полом.
        self.pad_z = (
            float(args.pad_z) if args.pad_z is not None
            else PAD_Z.get((self.pad[0], self.pad[1]), 0.0)
        )
        levels = [0.0, -self.pad_z] + [z - self.pad_z for z in parse_levels(args.levels)]
        self.terrain = Terrain(
            step=args.terrain_step, settle=args.terrain_settle, gap=args.terrain_gap,
            climb_speed=args.climb_speed, ground_max=args.ground_max, levels=levels,
        )
        self._alt_off = ""            # причина, по которой удержание высоты выключено
        self._alt_fails = 0
        self._alt_cmd = 0.0           # когда последний раз правили высоту
        self._alt_unsure = False      # уровень поверхности не опознан, о чём сказано
        self._alt_dirty = False       # поправка не прошла: повторить на ближайшем такте
    ...
    def status(self) -> dict:
        ...
        if self.alt_hold:
            # Что борт думает про высоту: сколько под ним до поверхности, насколько
            # эта поверхность выше взлётной площадки и сколько получается над самой
            # площадкой. Диспетчеру и человеку в пульте это единственный способ
            # увидеть, что дрон стоит над домом, а не набрал лишнего.
            agl = self.terrain.agl
            st["agl"] = None if agl is None else round(agl, 2)
            st["ground"] = round(self.terrain.ground, 2)
            st["pad_z"] = round(self.pad_z, 3)
            st["terrain_steps"] = self.terrain.steps
            if not self.terrain.known:
                st["terrain_warning"] = "уровень поверхности не опознан: дрейф не правлю"
            if self.terrain.clipped:
                st["terrain_warning"] = "уровень поверхности упёрся в предел правдоподобия"
        elif self._alt_off:
            st["alt_hold_off"] = self._alt_off
    ...
    def _telemetry(self, frame_id: str = "body", timeout: float = 2.0):
        ...
        frame_id выбирает, в чём отвечает борт: "body" — положение относительно
        корпуса, "terrain" — высота над поверхностью прямо под дроном (дальномер),
        "map" — мировая высота от точки включения питания.
        """
        box: dict = {}
    ...
    def alt_hold(self) -> bool:
        """Держим ли высоту по дальномеру.

        Выключается ключом --no-alt-hold, режимом --dry (там нет ни дальномера, ни
        высоты) и самим бортом — если замеров нет столько раз подряд, что держать
        высоту не по чему. Причина отключения остаётся в _alt_off и уходит в статус:
        молча переставший держать высоту дрон хуже, чем не державший её никогда.
        """
        return not self.args.no_alt_hold and not self.dry and not self._alt_off

    def _ground_gap(self) -> float | None:
        """Расстояние до поверхности прямо под дроном, м. None — замера нет.

        Это фрейм terrain из документации: не высота над полом, а высота над тем, что
        под дроном сейчас, — над домом она меньше на высоту дома. Замер идёт через ту
        же очередь к железу, что кадры и команды: два потока на одном ROS-узле рвут
        wait set.
        """
        try:
            tel, err = self._hw(
                "высота",
                lambda: self._telemetry("terrain", timeout=self.args.alt_timeout),
                timeout=self.args.alt_timeout,
            )
        except Busy:
            return None                 # борт занят своим делом, замер просто пропущен
        if tel is None:
            self._alt_note(err)
            return None
        height = getattr(tel, "z", None)
        if height is None:
            self._alt_note("в телеметрии нет z")
            return None
        self._alt_fails = 0
        return float(height)

    def _alt_note(self, why: str) -> None:
        """Замера нет. Считаем отказы подряд и на N-м выключаем удержание высоты."""
        self._alt_fails += 1
        if self._alt_fails == 1:
            say(f"высота: замера нет ({why})")
        if self._alt_fails >= self.args.alt_fails and not self._alt_off:
            self._alt_off = f"дальномер молчит {self._alt_fails} раз подряд: {why}"
            say(f"ВЫСОТУ НЕ ДЕРЖУ: {self._alt_off}. Полёт продолжается как раньше — "
                f"высота будет такой, какой её держит автопилот")

    def _push_altitude(self, target: float) -> bool:
        """Одна команда «висеть в target метрах над тем, что под тобой». True — отдана.

        Именно set_altitude, а не navigate: он меняет только высоту, не трогая ни
        X/Y, ни курс, и не переинициализирует траекторию перелёта. И именно во фрейме
        terrain — в том же, в котором ошибается автопилот. Просить у него «полтора
        метра над поверхностью», стоя над домом, значит велеть подняться на крышу;
        мы просим ровно столько, сколько нужно, чтобы высота над полем не изменилась.

        Мимо start(): поправка высоты — не команда борта, а короткое обращение к
        железу. Через start() она во время перелёта получила бы отказ «занят», а с
        preempt=True отменила бы сам перелёт — тогда как менять высоту прямо в полёте
        документация для set_altitude прямо разрешает. Ошибка здесь ничего не рушит:
        такт закончится, и следующий попробует снова.
        """
        try:
            resp = self._hw("высота", lambda: self.drone.control.set_altitude(
                z=float(target), frame_id="terrain",
            ), timeout=self.args.alt_timeout)
        except Busy:
            return False                # борт занят кадром или выдачей команды
        except AttributeError:
            # Сборка ноды без set_altitude. Обойти это navigate-ом нельзя: он
            # переинициализирует траекторию, и лечение вышло бы хуже болезни.
            self._alt_off = "на этой сборке нет команды set_altitude"
            say(f"ВЫСОТУ НЕ ДЕРЖУ: {self._alt_off}")
            return False
        except Exception as exc:  # noqa: BLE001 — отказ железа не должен ронять поток
            say(f"высота: команда не прошла ({exc})")
            return False
        if resp is not None and not getattr(resp, "success", True):
            say(f"высота: команда не принята ({getattr(resp, 'message', '')})")
            return False
        return True

    def alt_keeper(self) -> None:
        """Держать высоту над полем, не поднимаясь на крыши домов.

        Такт: замер до поверхности → решение, что изменилось → при необходимости одна
        команда высоты. Замер берётся часто (--alt-look): на редких замерах любой
        скачок выглядит плавным, а именно скачок и надо поймать.

        Работаем и во время перелёта — край крыши дрон пересекает именно там, и
        прежняя версия, дремавшая до конца команды, весь этот участок пропускала.
        """
        if self.args.alt_look <= 0 or self.dry:
            return
        while True:
            time.sleep(self.args.alt_look)
            self._alt_tick()

    def _alt_tick(self) -> str:
        """Один такт удержания высоты. Возвращает, чем такт кончился (для тестов).

        Отдельным методом, потому что весь смысл — в решении «дом это или дрейф», а
        проверять его на живом борте дорого: ступенька рельефа воспроизводится только
        полётом над краем дома.
        """
        # Единственное условие — дрон в воздухе и висит. Взлёт и посадка меняют
        # высоту намеренно, там держать нечего; занятость борта (self._busy) больше
        # не помеха: под перелётом крыша и кончается.
        if not self.alt_hold or self.state != "hover":
            return "idle"

        height = self._ground_gap()
        if height is None:
            return "no-data"
        kind = self.terrain.update(height, time.monotonic(), self.alt)
        if kind in ("bad", "first"):
            return kind

        target = self.terrain.target(self.alt, self.args.min_clear, self.args.max_alt)
        if kind == "step":
            # Ступенька: под дроном сменилась поверхность. Высоту над полем он не
            # терял, менять её не нужно — но и молчать нельзя: автопилот принимает
            # скачок замера за собственный уход и сам пойдёт его «исправлять».
            # Поэтому команду отдаём немедленно, мимо всех задержек, и означает она
            # «стой где стоишь».
            say(f"под дроном {'дом' if self.terrain.ground > 0 else 'провал'} "
                f"{abs(self.terrain.ground):.2f} м: до поверхности {height:.2f} м, "
                f"над полем те же {self.terrain.agl:.2f} м — высоту не меняю "
                f"(держу {target:.2f} м над тем, что под дроном)")
            return self._retarget(target, kind)

        if kind == "gap" or not self.terrain.known:
            # Уровень поверхности не опознан: дрон на кромке дома, либо карта поля
            # расходится с настоящей. Скачки ловить продолжаем, а дрейф не правим —
            # поправка от неизвестного уровня увела бы дрон вернее, чем сам дрейф.
            if not self._alt_unsure:
                self._alt_unsure = True
                say(f"высота: до поверхности {height:.2f} м не ложится ни на один "
                    f"известный уровень — дрейф не правлю, скачки ловлю")
            return "unsure"
        self._alt_unsure = False
        if self._alt_dirty:
            # Прошлая поправка не прошла: борт был занят кадром или выдачей команды.
            # Забыть о ней нельзя — у автопилота осталась цель от прежней поверхности,
            # и он всё это время уводит дрон. Цель пересчитана от свежего уровня.
            return self._retarget(target, "retry")
        if kind == "resync":
            return kind     # уровень восстановлен, дрейф правим со следующего такта
        if self._busy:
            return "flying"  # в перелёте правим только рельеф, маршруту не мешаем

        error = target - height
        if abs(error) < self.args.alt_dead:
            return "hold"
        now = time.monotonic()
        if now - self._last_move < self.args.alt_period:
            return "moving"             # дрон только что летел: дать ему успокоиться
        if now - self._alt_cmd < self.args.alt_period:
            return "too-soon"           # не чаще одной поправки в период
        say(f"высота: ушёл на {-error:+.2f} м (до поверхности {height:.2f} м "
            f"при {target:.2f} м) — возвращаю")
        return self._retarget(target, kind)

    def _retarget(self, target: float, kind: str) -> str:
        """Отдать цель по высоте и запомнить, отдалась ли она.

        Неудача — не «ну и ладно»: у автопилота остаётся цель от прежней поверхности,
        по которой он и уводит дрон. Поэтому она помечается и повторяется на каждом
        следующем такте, пока не пройдёт.
        """
        if not self._push_altitude(target):
            self._alt_dirty = True
            return "busy"
        self._alt_dirty = False
        self._alt_cmd = time.monotonic()
        return kind
    ...
    def _takeoff(self, alt: float, job: Job) -> None:
        ...
        self.last_error = ""
        # Нулём высоты рельефа объявляется своя площадка: она у нас на крыше дома, и
        # «полтора метра» монитора отсчитываются от неё, а не от пола поля.
        self.terrain.reset()
    ...
    def _land(self, job: Job) -> None:
        ...
        # до посадки, чтобы фоновый доворот не крутил дрон, пока тот садится. С
        # высотой то же самое: снижающийся дрон меняет её сам, и поправлять её —
        # значит мешать посадке.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="не держать высоту по дальномеру (по умолчанию держим: поле разновысотное)",
    )
    p.add_argument("--alt-look", type=float, default=0.1,
                   help="как часто мерить расстояние до поверхности, с (0 — не мерить)")
    p.add_argument("--alt-period", type=float, default=3.0,
                   help="не чаще одной поправки ДРЕЙФА в N с и не раньше N с после "
                        "движения; скачок рельефа правится сразу, минуя это")
    p.add_argument("--alt-dead", type=float, default=0.12,
                   help="уход высоты меньше этого не трогаем: шум дальномера, м")
    p.add_argument("--alt-timeout", type=float, default=0.5,
                   help="сколько ждать замер высоты, с (дольше — считаем, что его нет)")
    p.add_argument("--alt-fails", type=int, default=5,
                   help="столько замеров подряд не получено — перестаём держать высоту")
    p.add_argument("--pad-z", type=float, default=None,
                   help="высота своей площадки над полом поля, м; по умолчанию из карты "
                        "поля по --cell. ПЕРЕМЕРИТЬ РУЛЕТКОЙ в первый день")
    p.add_argument("--levels", default="",
                   help="прочие высоты поверхностей поля над полом через запятую, м "
                        "(нужно только с --allow-goto: пол и своя крыша уже учтены)")
    p.add_argument("--terrain-step", type=float, default=0.25,
                   help="скачок замера больше этого — не дрон, а другая поверхность под ним, м")
    p.add_argument("--terrain-settle", type=float, default=0.15,
                   help="насколько уровень поверхности может отличаться от известного "
                        "по карте, чтобы считаться им же, м")
    p.add_argument("--terrain-gap", type=float, default=1.5,
                   help="перерыв в замерах дольше этого — уровень опознаём заново, с")
    p.add_argument("--min-clear", type=float, default=0.4,
                   help="ближе этого к поверхности под дроном не опускаться, м")
    p.add_argument("--ground-max", type=float, default=1.5,
                   help="предел правдоподобия для высоты поверхности под дроном, м")
...
def main(argv: list[str] | None = None) -> int:
    ...
    threading.Thread(target=agent.alt_keeper, daemon=True).start()

    if agent.alt_hold:
        say(f"высоту держу по дальномеру: замер раз в {args.alt_look:g} с, "
            f"скачок больше {args.terrain_step:g} м считаю сменой поверхности под дроном, "
            f"а не уходом высоты")
        say(f"своя площадка на {agent.pad_z:g} м над полом"
            + (" (из карты поля — перемерить рулеткой)" if args.pad_z is None else " (задано ключом)")
            + f", уровни под дроном: {', '.join(f'{v:+.2f}' for v in agent.terrain.levels)} м от неё")
    elif args.no_alt_hold and not args.dry:
        say("высоту не держу: запущен с --no-alt-hold, высота остаётся на автопилоте")
    if agent.yaw_hold:
